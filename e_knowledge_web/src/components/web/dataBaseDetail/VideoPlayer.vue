<template>
  <video-player class="vjs-custom-skin custom-video" ref="videoPlayer" :options="playerOptions" @play="onPlayerPlay($event)" @pause="onPlayerPause($event)" @ended="onPlayerEnded($event)" @loadeddata="onPlayerLoadeddata($event)" @waiting="onPlayerWaiting($event)" @playing="onPlayerPlaying($event)" @timeupdate="onPlayerTimeupdate($event)" @canplay="onPlayerCanplay($event)" @canplaythrough="onPlayerCanplaythrough($event)" @ready="playerReadied" @statechanged="playerStateChanged($event)">
  </video-player>
</template>
<script>
  ...
  data() {
    return {
      playerOptions: {
        // component options
        start: 0, //视频默认开始时间
        playsinline: false, //set player not full-screen in mobile device
