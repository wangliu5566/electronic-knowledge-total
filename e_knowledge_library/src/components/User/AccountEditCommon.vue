    ...
    <Row class="pt20">
      <div class="form-container">
        <Form ref="accountAddFromHeader" :model="accountDatas" inline :label-width="100" :rules="accountRules">
         ...
         <!--  <Form-item label="机构" style="width:48%;" required >
            <Input type="text" v-model="showCompany" disabled></Input>
          </Form-item> -->
...
export default {
  ...
  mounted() {
    this.checkIsAdd();
    this.showCompany = JSON.parse(window.sessionStorage.getItem('bg_user_info')).agency;
  },
  methods: {
    checkIsAdd() {
      ...
      if (index != -1) {  //新增
        ...
        this.coverTimer = window.setInterval(()=>{
         this.userCover = window.sessionStorage.getItem('user-big-cover');
        }, 2000)
      } else {
        this.isAdd = false;
        this.coverTimer = window.setInterval(()=> {
    ...
    goBackAccount() {
      this.$router.push({path: '/wrap/system/account'})
    },
    cancelHandle() {
      this.$router.push({path: '/wrap/system/account'})
    },
    ...
    submitAddAccountData() {
      this.$http.post('/v1/admin/user/create',
        Object.assign({}, this.accountDatas, {
          ...
          company: JSON.parse(window.sessionStorage.getItem('bg_user_info')).agency,
          department: '产品部',
          role: '任务管理员',
