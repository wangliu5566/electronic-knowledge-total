        ...
        <Form ref="addNewAcademyForm" :model="addNewAcademyDatas" :label-width="80" :rules="addNewAcademyRules">
          ...
          <FormItem label="IP段" required>
            <div v-for="(item,index) in addNewAcademyDatas.ips" key='index' class="edit-ip-row">
              <Row>
                <Col span="11">
                <FormItem :prop="'ips.' + index + '.startIp'" :rules="{required: true, message: '该项不能为空', trigger: 'blur'}">
                  <Input v-model="item.startIp" placeholder="请输入开始IP"></Input>
                </FormItem>
                </Col>
                <Col span="1" style="text-align: center">-</Col>
                <Col span="11">
                <FormItem :prop="'ips.' + index + '.endIp'" :rules="{required: true, message: '该项不能为空', trigger: 'blur'}">
            ...
            <Row class="pt20">
              <Button type="info" long @click="addNewIp">新增IP段</Button>
            </Row>
          </FormItem>
        </Form>
      </div>
